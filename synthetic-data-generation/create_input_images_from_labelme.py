    ...
    for json_file in tqdm.tqdm(labelme_json_paths):
        ...
        for ext in IMG_EXT:
            if os.path.exists(os.path.join(input_dir, os.path.splitext(os.path.basename(json_file))[0] + ext)):
                image_path = os.path.join(input_dir, os.path.splitext(os.path.basename(json_file))[0] + ext)
                break
        ...
        for annotation in data['shapes']:
            ...
            new_im.save(os.path.join(output_dir, label, f'{label_counts[label]}-{original_name}.png'), optimize=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--margin', type=int, default=10, help='Margin (in pixels) to include around cropped objects.')
    args = parser.parse_args()

    extract_objects_from_labelme_data(args.input_dir, args.output_dir, margin=args.margin)
